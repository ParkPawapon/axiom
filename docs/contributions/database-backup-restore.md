...
- Remote backup destinations can copy managed backup, metadata, and signature artifacts to an absolute mounted path, S3 URI, R2 URI through an `AXIOM_R2_ENDPOINT_URL`, GCS URI, or SFTP URI.
- S3, R2, and GCS support native backend uploads through scoped environment credentials with CLI fallback; SFTP remains an allowlisted CLI destination.
...
- Continuous replay restore can restore a base managed artifact and then replay sorted SQL recovery segments, MySQL binlog files converted through `mysqlbinlog`, PostgreSQL WAL-derived `.wal.sql` replay segments, and PostgreSQL physical WAL restore manifests.
...
- macOS LaunchAgent, Windows Task Scheduler, and Linux systemd user timer adapters are prepared behind a scheduler port so background backup sweeps can run after the desktop app is closed.
...
- Destructive SQL rollback requires explicit `-- axiom:rollback ...` annotations instead of unsafe guessing.
- Backup trust enrollment can export/import a signing-key fingerprint bundle, explicitly enroll a selected artifact hash, and optionally carry a passphrase-protected recovery key for cross-machine restore without exposing raw secret key material.
...
- Trust bundles and artifact enrollment must contain fingerprints, artifact hashes, or passphrase-encrypted recovery keys only, never plaintext backup signing or encryption key material.
- KMS envelope payloads must never expose plaintext data keys to frontend state or logs.

## Still Conservative By Design

- SFTP remote copy remains CLI-backed.
- Complex destructive migration rollback is not generated without explicit rollback annotations.
- External provider credentials must come from environment, platform secure storage, or provider CLIs, never frontend state.
