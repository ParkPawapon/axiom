use crate::domain::project::project_docker::{ProjectDockerAction, ProjectDockerActionResult};
...
pub fn generate_project_docker_compose(
    ...
    let status = docker_client.get_project_status(&project)?;

    Ok(ProjectDockerActionResult {
        project_id: config.project_id,
        action: ProjectDockerAction::Generate,
        status,
